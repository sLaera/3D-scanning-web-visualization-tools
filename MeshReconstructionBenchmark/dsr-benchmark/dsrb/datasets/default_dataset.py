...
class DefaultDataset:
    ...
    def __init__(self,
        ...
        if not os.path.isfile(data_dir_file):
            self.logger.error(
                "Could not find {}. Please put a file called DATA_DIR.txt with the path that points to your dataset directory in the root folder of this project.".format(data_dir_file))
            return 1
        ...
        if not os.path.isfile(cpp_dir_file):
            self.logger.error(
                "Could not find {}. Please put a file called CPP_DIR.txt with the path that points to your cpp directory in the root folder of this project.".format(cpp_dir_file))
        else:
        ...
        self.path = data_dir
        self.model_dicts = []
        self.mesh_tools_dir = os.path.join(
            cpp_dir, "mesh-tools", "build", "release")
        self.poisson_exe = os.path.join(
            cpp_dir, "PoissonRecon", "Bin", "Linux", "PoissonRecon")
        self.labatut_exe = os.path.join(self.mesh_tools_dir, "labatut")
        self.lidar_exe = os.path.join(
            cpp_dir, "synthetic-lidar-unity", "Build", "Build.x86_64")
            ...
            for model in tqdm(models, ncols=50, file=sys.stdout):
                ...
                try:
                    ...
                    os.makedirs(os.path.dirname(
                        model["scan_ply"]), exist_ok=True)
                    os.rename(model["scan"].replace(
                        ".npz", ".ply"), model["scan_ply"])
    ...
    def sample(self, n_points=1000000, overwrite=True):
        ...
        for m in tqdm(self.model_dicts, disable=self.tqdm_disabled, file=sys.stdout):
            ...
            try:
                ...
                np.savez_compressed(
                    m["pointcloud"], points=points, normals=normals)
    ...
    def make_pointcloud_ply(self, n_points=100000, std_noise=0.0):
        ...
        for m in tqdm(self.model_dicts, file=sys.stdout):
            ...
            ind = np.random.randint(
                0, data["points"].shape[0], size=(n_points,))
    ...
    def standardize(self, padding=0.1):
        ...
        for m in tqdm(self.model_dicts, disable=self.tqdm_disabled, file=sys.stdout):
            ...
            print("Standardize {} to {}".format(
                m["mesh"], os.path.splitext(m["mesh"])[0]+"_unit.off"))
            ...
            max_bound = np.vstack(
                [np.abs(mesh.get_min_bound()), mesh.get_max_bound()])
            col_index = np.argmax(max_bound, axis=1)[1]
            scale = np.abs(mesh.get_min_bound()[
                           col_index]) + np.abs(mesh.get_max_bound()[col_index])

            mesh = mesh.scale((100-padding) / scale, [0, 0, 0])

            o3d.io.write_triangle_mesh(
                os.path.splitext(m["mesh"])[0]+"_unit.off", mesh)
    ...
    def scale(self):
        ...
        for m in tqdm(self.model_dicts, disable=self.tqdm_disabled, file=sys.stdout):
            ...
            outfile = os.path.join(
                self.path, "input_unit", os.path.split(m["mesh"])[1])
    ...
    def recolor_in_cells(self, method, colors="pale", backend="vedo"):
        ...
        if isinstance(colors, str):
            if colors == "saturated":
                colors = ["#ff0000", "#f4ec00", "#01ffff", "#ff7f00",
                          "#0000ff", "#00ff01", "#6f00d8", "#ff00ff"]
            elif colors == "pale":
                colors = ["#CC99C9", "#9EC1CF", "#9EE09E",
                          "#FDFD97", "#FEB144", "#FF6663"]
            else:
    ...
    def recolor_planes(self, colors="pale"):
        ...
        for model in tqdm(self.model_dicts, file=sys.stdout):
            ...
            pe.save_points_and_planes_from_array(
                plane_filename=plane_file, point_filename=pt_file, planes_array=data)
    ...
    def ply2npz(self):
        ...
        for model in self.model_dicts:
            ...
            np.savez_compressed(model["pointcloud"], points=np.asarray(
                pc.points), normals=np.asarray(pc.normals))
