...
double pcaKNN(Point_set& point_set){
    ...
    auto noise_opt = point_set.property_map<double>("noise");

    if (!noise_opt.has_value()) {
        throw std::runtime_error("Property 'noise' not found in point set.");
    }

    Point_set::Property_map<double> noise = noise_opt.value();
