...
class BergerTest(DefaultDataset):
    '''
    This is the Berger Dataset class used to test the correct installation of the components.
    This is an edited Copy of Berger class in dsr-benchmark/dsrb/datasets/Beger.py
    '''
    def __init__(self, path=None, classes=[]):
        """Construct the dataset class

        Args:
            path (str, optional): Optional path to the dataset folder. Defaults to None.
            classes (list, optional): Optional list of classes of Berger. Used to select a subsaple of classes. Defaults to [].
        """        
        super().__init__()
        if path is not None:
            self.path = path
    ...
    def get_models(self, scan_configuration=["0", "1", "2", "3", "4"], names=None):
        """Compute preliminary operations and generate a dictionary of all the usefull paths to the models

        Args:
            scan_configuration (list, optional): scan configurations. Defaults to ["0", "1", "2", "3", "4"].
            names (list<str>, optional): list of a subset of classes to get. Defaults to None.

        Returns:
            list: paths of the models
        """        
        self.scan_conf = scan_configuration if isinstance(
            scan_configuration, list) else [scan_configuration]
        ...
        for s in self.scan_conf:
            for c in self.classes:
                ...
                d["scan"] = os.path.join(
                    self.path, "scan", "{}_{}.npz".format(c, s))
                d["scan_ply"] = os.path.join(
                    self.path, "scan_ply", "with_normals", "{}_{}.ply".format(c, s))

                d["convex_hull"] = os.path.join(
                    self.path, "p2m", "convex_hull", s, c+".obj")
                d["poisson_6"] = os.path.join(
                    self.path, "p2m", "poisson", s, c+".ply")
                ...
                d["pointcloud_ply"] = d["eval"]["pointcloud"].replace(
                    ".npz", ".ply")

                d["mesh"] = os.path.join(self.path, "mesh", "{}.off".format(c))

                d["planes"] = os.path.join(
                    self.path, "planes", c, s, "planes.npz")
                d["plane_params"] = os.path.join(
                    self.path, "planes", c, s, "params.txt")

                d["output"] = {}
                d["output"]["surface"] = os.path.join(
                    self.path, '{}', c, s, "surface.obj")
                d["output"]["surface_simplified"] = os.path.join(
                    self.path, '{}', c, s, "surface_simplified.obj")
                d["output"]["partition"] = os.path.join(
                    self.path, '{}', c, s, "partition.ply")
                d["output"]["partition_pickle"] = os.path.join(
                    self.path, '{}', c, s, "partition")
    ...
    def test_scan(self, scan_configuration="4"):
        print("Checking scans")
        # check that files are already scanned
        missing_file = False
        ...
        for s in self.splits:
            ...
            for model in tqdm(models, ncols=50, file=sys.stdout):
                file_path = str(model["scan"][:-4]) + ".npz"
                if not os.path.exists(file_path):
                    missing_file = True
                    break
        if missing_file:
            # run the scan again
            self.scan(scan_configuration)
        else:
            print("All scanned files are already present. Skipped")
    ...
    def scan_berger(self, args):
        ...
        conf_path = os.path.join(
            args.working_dir, "confs/bumps_" + str(args.conf) + ".cnf")
        ...
        for m in tqdm(self.model_dicts, ncols=50):
            try:
                command = [str(os.path.join(self.mesh_tools_dir, "normal")),
                           "-w", str(self.path),
                           "-s", "npz",
                           "-i", str(os.path.join("scan",
                                     m["model"], self.scan_conf + ".npz")),
                           "-o", str(os.path.join("scan",
                                     m["model"], self.scan_conf)),
                           "--method", method,
    ...
    def makePoisson(self, depth=8, boundary=2, outf=lambda m, self: os.path.join(self.path, m["class"], m["mesh"])):
        ...
        for s in self.splits:
            ...
            for m in tqdm(models, ncols=50):
                # try:
                os.makedirs(os.path.dirname(outf(m, self)), exist_ok=True)
                ...
                mesh = o3d.io.read_triangle_mesh(outf(m, self) + ".ply")
                o3d.io.write_triangle_mesh(outf(m, self), mesh)

    def makeLabatut(self, outf=lambda m, self: os.path.join(self.path, m["class"], m["mesh"])):
        sigma = 0.001
        lam = 5
        alpha = 32
        if self.splits is None:
        ...
        for s in self.splits:
            ...
            for m in tqdm(models, ncols=50):
                ...
                mesh = o3d.io.read_triangle_mesh(outf(m, self) + "_rt_5.0.ply")
                o3d.io.write_triangle_mesh(outf(m, self), mesh)
    ...
    def makeEval(self, n_points=100000, padding=0.1):
        ...
        for m in tqdm(self.model_dicts):
            ...
            np.savez(m["pointcloud"], points=points_surface,
                     normals=normals, loc=loc, scale=scale)
            ...
            if DEBUG:
                ...
                o3d.io.write_point_cloud(
                    str(Path(m["pointcloud"]).with_suffix(".ply")), pcd)
            ...
            np.savez(m["occ"], points=points,
                     occupancies=occupancies, loc=loc, scale=scale)
            ...
            if DEBUG:
                ...
                o3d.io.write_point_cloud(
                    str(Path(m["occ"]).with_suffix(".ply")), pcd)
...
if __name__ == '__main__':
    ...
    models = ["cube", "split_cube", "double_split_cube",
              "slanted_cube", "double_slanted_cube", "complex_cube"]
    ds = BergerTest(classes=models)
