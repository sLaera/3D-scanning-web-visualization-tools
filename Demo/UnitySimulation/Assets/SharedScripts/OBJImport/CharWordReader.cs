...
namespace SharedScripts.OBJImport {
	public class CharWordReader {
		...
		public Vector3[] ReadVectors(int numberOfVectorsToRead, out int numberOfVectorsRed) {
			SkipWhitespaces(out var newLinePassed);
			var vectors = new Vector3[numberOfVectorsToRead];
			int i = 0;
			while (!newLinePassed && i < numberOfVectorsToRead)
			{
				vectors[i] = ReadVector();
				i++;
				SkipWhitespaces(out newLinePassed);
			}
			numberOfVectorsRed = i;
			return vectors;
		}
