...
const updateSimulation = (data: TelemetryData): void => {
  //update simulation when the history is updated
  if (data) channel.value?.call('GameManager', 'UpdateSimulation', JSON.stringify(data))
}
const { startStream, isStreamEnded, createOffFile, sendOffFileSnippet } = useTelemetry(updateSimulation)

watch(isStreamEnded, (newIsStramEnded: boolean) => {
...
function loaded(c: UnityChannel) {
  ...
  c.subscribe('modelLoaded', (modelUrl) => {
    if (typeof modelUrl === 'string') {
      ...
      console.error({ modelUrl }, 'Type not valid')
    }
  })

  c.subscribe('offFileSnippet', (snippet) => {
    if (typeof snippet === 'string') {
      sendOffFileSnippet(snippet)
    } else {
      throw new Error(`${{ snippet }}, Type not valid`)
    }
  })

  c.subscribe('createOffFile', (numberOfPoints) => {
    if (typeof numberOfPoints === 'number') {
      createOffFile(numberOfPoints)
    } else {
      throw new Error(`${{ numberOfPoints }}, Type not valid`)
    }
...
defineExpose<{ loadMesh: (meshInfo: ModelSelectOption) => void; startSimulation: () => void }>({
  loadMesh,
  startSimulation
})
</script>
